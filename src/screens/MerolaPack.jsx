...
export default function MerolaPack({ section, index = 0 }) {
  if ((section === 'lessons' || section === 'parts') && index !== null) {
    ...
    if (!group) return <main className="page"><Back /><p className="muted">That content group was not found.</p></main>;
    return <main className="page"><Back /><div className="col"><span className="pill">Merola library</span><h1>{group.title}</h1><p className="muted">{group.instr}</p></div><QuizRunner items={group.items} grade="G3" subject="vocab" title="Practice" /></main>;
  }

  if (section === 'stories' && index !== null) {
    const story = merolaStories[index];
    if (!story) return <main className="page"><Back /><p className="muted">That story was not found.</p></main>;
    return <main className="page"><Back /><article className="card story"><span className="pill">Merola reading</span><h1>{story.title}</h1><p>{story.passage}</p></article><QuizRunner items={story.items} grade="G3" subject="reading" title="Check your reading" /></main>;
  }

  if (section === 'words') return <main className="page"><Back /><div className="col"><span className="pill">Merola library</span><h1>Reading and spelling bank</h1><p className="muted">{readingSentences.length} reading sentences and {spellingWords.length} spelling words from the Merola collection.</p></div><section className="card"><h2>Reading sentences</h2><div className="grid g3">{readingSentences.map((sentence, index) => <p className="soft" style={{ padding: 10, margin: 0 }} key={`${sentence}-${index}`}>{sentence}</p>)}</div></section><section className="card"><h2>Spelling words</h2><div className="chips">{spellingWords.map((word, index) => <span className="pill" key={`${word}-${index}`}>{word}</span>)}</div></section></main>;
  ...
  if (section === 'lessons' || section === 'parts' || section === 'stories') {
    ...
    return <main className="page"><Back /><div className="col"><span className="pill">Merola library</span><h1>{section === 'lessons' ? 'Lesson quizzes' : section === 'parts' ? 'Word parts' : 'Reading stories'}</h1><p className="muted">Choose a group to start.</p></div><GroupList groups={groups} kind={kind} label={section} /></main>;
  }

  return <main className="page"><section className="hero"><span className="emoji">📚</span><span className="pill w">Imported content collection</span><h1>Merola library</h1><p>More lessons, stories, word parts, reading sentences and spelling practice inside Kidsy.</p></section><div className="grid g4">{tabs.map(([key, label, Icon]) => <Link key={key} to={`/merola/${key}`} className="tile t2"><Icon className="ico" /><b>{label}</b><span>{key === 'lessons' ? `${merolaCounts.lessons} groups · ${merolaCounts.lessonQuestions} questions` : key === 'stories' ? `${merolaCounts.stories} stories` : key === 'parts' ? `${merolaCounts.wordParts} word-part groups` : `${merolaCounts.readingSentences} sentences · ${merolaCounts.spellingWords} words`}</span></Link>)}</div>{!section && <><h2>Lesson quizzes</h2><GroupList groups={merolaLessons.slice(0, 4)} kind="lessons" label="Lesson quizzes" /><h2>Reading stories</h2><GroupList groups={merolaStories} kind="stories" label="Reading stories" /></>}</main>;
}